...
OFDMDemod(ofdmMod,waveform(:,3),modOrder,false,title);
...
TsVect=0:Ts:Ts*(length(waveform(:,1))-1);
    ...
    for antenna=1:1:chTaps(1)
        for path=1:1:chTaps(3)
            ...
            inY=interp1(TsVect,waveform(:,3),inX,'pship');
            chBS_T(antenna,:)=inY*chan(1).coeff(antenna,1,path,1)+chBS_T(antenna,:);%channel bs-terminal (only MP, no interf)
...
optimalWeight1 = LMSalgorithm(chOutQUADRIGA,waveform(:,3),numArrayElements,Pars.lambda);  
...
% path_loss_i2 = 1;
  ...
  receivedW = collectPlaneWave(g.BSarray, [waveform(:,3)*(1/sqrt(path_loss_t1))], [t1Angles'], Pars.fc);
...
optimalWeight1 = LMSalgorithm(chOutLOS,waveform(:,3),numArrayElements,Pars.lambda);  
