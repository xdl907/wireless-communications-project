...
while(1)
    ...
    Geometry.BSarray = phased.URA('Size', [4 4], 'ElementSpacing', [Pars.lambda/2 Pars.lambda/2], 'ArrayNormal', 'x');
    ...
    %% algoritmo LMS
    % definizione del complesso coniugato del segnale in input
    chOutcc = conj(chOut);
    % definizione della matrice di correlazione
    y1ctrans = transpose(chOutcc);
    Y = chOut' * y1ctrans';
    R = corr(Y); % tutto a posto
    % definizione di z
    %zpreprod = y1 * transpose(y1s);
    z = corr(chOut, sinusoid_waveform_t1');
    
    % definizione del gradiente di MSE
    MSEgrad = (w1'*R.*2)-(z.*2);
    % definisco il valore mu
    mu = 1 / sum(eig(R));
    
    % definisco il vettore peso finale
    OptimalWeight = zeros(size(w1s));
    for i = 1 : size(w1s)
       if i == 1        % non sono sicuro serva, nel caso io lo metto
          OptimalWeight(i) = 1;
       else
           OptimalWeight(i) = OptimalWeight(i-1) - 0.5 * MSEgrad(i-1) * mu;
       end           
    end
    %% fine ciclo while
