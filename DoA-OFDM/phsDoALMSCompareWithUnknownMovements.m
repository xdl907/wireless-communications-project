...
for c = 1:60
    ...
    hold on;
    plot3(v(1,(1)),v(2,(1)),v(3,(1)),'ob','LineWidth',1.5); %tracked terminals
    plot3(v(1,(2)),v(2,(2)),v(3,(2)),'or','LineWidth',1.5); %tracked terminals
...
%     receivedW = collectPlaneWave(Geometry.BSarray, [waveform_t1*(1/sqrt(path_loss_t1)) waveform_t2*(1/sqrt(path_loss_t2)) waveform_i*(1/sqrt(path_loss_i1)) waveform_i*(1/sqrt(path_loss_i2))], [t1Angles' t2Angles' i1Angles' i2Angles'], Pars.fc);
    ...
    %beamforming UE1 con un angolo reale
    ...
    bits=OFDMDemod(ofdmMod,y1real,modOrder,true,title);
    [numbError,ratio]=biterr(in_t1,bits);
    ...
    [numbError,ratio]=biterr(in_t2,bits);
    fprintf('\nNumber of errors phase shift 2 using DoA: %d',numbError);
    fprintf('\nBER phase shift 2: %f',ratio);
    
    title='Constellation with PhaseShiftBeamformer on UE2 using real angles';
    bits=OFDMDemod(ofdmMod,y2real,modOrder,true,title);
    [numbError,ratio]=biterr(in_t2,bits);
    fprintf('\nNumber of errors phase shift 2 using Real Angles: %d',numbError);
    fprintf('\nBER phase shift 2: %f',ratio);
