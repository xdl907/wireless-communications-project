...
%     t1 = abs(t1+[-10+20*rand(2,1);0]); %random movements (between -5 and 5)
%     t2 = abs(t2+[-10+20*rand(2,1);0]);
%     i1 = abs(i1+[-10+20*rand(2,1);0]);
%     i2 = abs(i2+[-10+20*rand(2,1);0]);
   %end
    if ((t1(1) > 100) || (t1(2) > 100))
     ...
     if (mod(t2(1),100) == 0) || (mod(t2(2),100) == 0)
        t2 = abs(t2+[-5;-7;0]);
    else
        t2 = abs(t2+[5;7;0]);
     end
    ...
    'ElevationScanAngles', -90:0.05:90);
    ...
    title='Constellation with PhaseShiftBeamformer on UE1 using value of DoA';
    ...
    hold on
    %% stampa dei beam fatti con i pesi della funzione beamformer (su doa)
    ...
    %% algoritmo LMS 
    %calcolo dei pesi sul segnale originale
   ...
   hold on
    ...
    hold on 
    polarplot(H,'--b')
    legend ('UE1', 'UE2','array directivity pattern with PhaseShift BF on UE1 doa1','array directivity pattern with PhaseShift BF on UE2 doa2','array directivity pattern with LMS BF on UE1','array directivity pattern with LMS BF on UE2','Location','northeastoutside')
        
    hold off
        % for debugging reason only
%     number = num2str(c);
%     graph = "complete_polarplot";
%     filename = strcat(graph, number);
%     print(filename,'-dpng');
    pause(5);
